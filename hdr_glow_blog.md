---
title: "PNG なのに HDR で光る? PQ + ICC で部分グロー画像を作る仕組み"
date: 2026-06-06
tags: ["HDR", "PQ", "PNG", "Canvas", "ICC", "Twitter"]
summary: "8bit の普通の PNG を HDR ディスプレイで光らせる仕掛けを解剖する。PQ 符号化、ICC プロファイル埋め込み、そして X(Twitter)で iCCP が落ちないようにする工夫まで。"
---
...
- X に上げる時は **Web Share API でアプリへ直接渡す + 出力サイズを小さく** (~1080px 以下) しないと、サーバ側で再エンコされて `iCCP` が消える
...
PQ の EOTF⁻¹ (linear nits → 符号値) は次の式:

$$
V = \left( \frac{c_1 + c_2 Y^{m_1}}{1 + c_3 Y^{m_1}} \right)^{m_2}
$$

ここで `Y = L / 10000` (L は nits)、定数は仕様で固定されている:
...
元のひかるやつアプリは 2 つの工夫で生き残らせている:

1. **画像サイズを 400×400 に固定** — 再エンコの閾値を下回るので iCCP が保持されやすい
2. **`navigator.share({ files: [file] })` で X アプリに直接渡す** — Web 経由のアップロードよりも変換が緩い

ダウンロードしてから「写真を選択」で添付すると再エンコ確率が大幅に上がるので、**iOS Safari → 共有 → X アプリ** の経路が必須。

### 7.3 対策まとめ

| 罠 | 対策 |
|---|---|
| 大きい PNG → 再エンコで iCCP 脱落 | 出力サイズを **1080px 以下** (なるべく 400px) にリサイズしてから書き出す |
| Web Share 非経路 → 高確率で再エンコ | `navigator.canShare({files})` を必ず確認し、ファイル渡しで `navigator.share` |
